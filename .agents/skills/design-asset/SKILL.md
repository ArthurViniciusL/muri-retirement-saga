...
are not style preferences: this game has no colour, so tone, silhouette and line carry
100% of the visual information. Breaking one of them does not make the art *different*,
it makes the game unreadable.

## Before drawing anything

Read the rules that govern the asset you are about to make. They are short and they are
the source of truth — `guidelines.md` is the older document and the rules supersede it
wherever they disagree (`architecture-source-of-truth.md`).

| File | What it locks |
| --- | --- |
| `.agents/rules/art-palette-zinc.md` | The eleven allowed colours, and the 3–4 tones per sprite limit |
| `.agents/rules/art-grid-and-scale.md` | The locked 64 px grid and every asset size |
| `.agents/rules/art-linework-and-texture.md` | Outline, hatching, no gradient/glow/transparency, silhouette first |
...
1. **Zinc only.** The eleven tones in `ZINC` and nothing else. One hex outside the scale
   anywhere in the file is a failure.
2. **At most 4 tones per sprite.** Typically outline + shadow + light + one accent. Using
   more of the scale at once reads as a soft gradient, which is exactly the effect the
...
4. **Outline is mandatory** on every playable sprite, HUD icon, card and button: 1 px of
   the sprite's darkest tone (normally zinc-950) around the whole silhouette, so the
   asset reads against any background. Tiles are the exception — see the catalog.
5. **Shadow is hatching or dithering**, never interpolation. A mid tone between two zinc
   steps is made by alternating pixels, not by picking a colour in between.
...
Draw the silhouette before anything else, as an ASCII grid, and print it. In a game with
no colour, a sprite that is not recognisable in pure black against the background is
...
`canvas.outline("zinc-950")` grows the carve outwards, so the artwork needs at least
...
from pixelpng import Canvas, ZINC, save_frames
```

| Call | Use |
| --- | --- |
| `Canvas(w, h)` | Empty transparent canvas |
| `Canvas.from_ascii(rows, legend)` | Build from an ASCII grid; legend maps a char to a zinc tone or `None` |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| Sprite disappears against a platform | Playable sprite shaded in mid tones | Push the body into zinc-700..950 and keep decoration light |
